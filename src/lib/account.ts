...
class Account {
  ...
  async syncEmails() {
    ...
    try {
      ...
      // Si pas de nextDeltaToken, faire une sync initiale
      if (!account.nextDeltaToken) {
        console.log("Performing initial sync...");
        const initialSync = await this.performInitialSync();
        if (!initialSync) throw new Error("Initial sync failed");

        allEmails = initialSync.emails;
        storedDeltaToken = initialSync.deltaToken;
      } else {
        // Sync incrémentale
        ...
        storedDeltaToken = response.nextDeltaToken || account.nextDeltaToken;
