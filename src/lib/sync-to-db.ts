...
async function upsertEmail(
  ...
  try {
    ...
    // Modifiez la fonction upsertEmail :
    let emailLabelType: "inbox" | "sent" | "draft" = "inbox";
    // Vérifiez si c'est un email envoyé en comparant l'adresse de l'expéditeur avec l'adresse du compte
    if (email.from.address === email.from.address) {
      // email envoyé depuis ce compte
      emailLabelType = "sent";
    } else if (email.sysLabels.includes("draft")) {
      emailLabelType = "draft";
    } else {
